...
class OpenCVCOCOTennisPoseRealtimeVisualizer(IStreamingVisualizer):
    ...
    def render(
        ...
        if isinstance(data, COCOPoseTennisSequenceData):
            return self.render_stream(data.frames, context)
        if isinstance(data, COCOPoseTennisFrameData):
            return self.render_stream((data,), context)
        raise TypeError(
            "OpenCVCOCOTennisPoseRealtimeVisualizer requires COCOPoseTennisFrameData "
            f"or COCOPoseTennisSequenceData, got {type(data).__name__}."
        )
    ...
    def _build_canvas(self, pose_frame: COCOPoseTennisFrameData) -> np.ndarray:
    ...
    def _draw_pose(self, canvas: np.ndarray, pose_frame: COCOPoseTennisFrameData) -> None:
    ...
    def _draw_status(self, canvas: np.ndarray, pose_frame: COCOPoseTennisFrameData) -> None:
        movement = getattr(pose_frame, "tennis_movement", None)

        label = (
            f"frame: {pose_frame.frame_index} | "
            f"movement: {movement} | "
            f"ESC/q: close preview"
        )
    ...
    @staticmethod
    def _require_pose_frame(item: IData) -> COCOPoseTennisFrameData:
        if not isinstance(item, COCOPoseTennisFrameData):
            raise TypeError(
