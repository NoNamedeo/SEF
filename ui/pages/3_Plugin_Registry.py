...
for cat in shown_categories:
    ...
    with st.expander(f"**{label}** — {len(plugins)} plugin", expanded=True):
        if not plugins:
            ...
            continue
        rows = [{"Nome": p.name, "Descrizione": p.description, "Factory": p.factory.__name__} for p in sorted(plugins, key=lambda p: p.name)]
...
with st.form("register_plugin_form"):
    ...
    r_cls = st.text_input("Classe Python (percorso completo, es. `mymodule.MyTracker`)", key="rp_cls")
