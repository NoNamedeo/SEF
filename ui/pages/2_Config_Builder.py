...
st.caption(
    "Definisci la pipeline tramite un dizionario di configurazione. "
    "Il registry risolve i nomi nei componenti concreti."
)
...
with col_editor:
    ...
    validate_clicked = col_validate.button("🔍 Valida", use_container_width=True)
    if col_reset.button("↺ Reset default", use_container_width=True):
...
if st.button("▶ Costruisci ed esegui", type="primary", use_container_width=True, disabled=run_disabled):
    try:
        ctx = context_from_config(parsed_cfg, registry)
    except Exception as exc:
