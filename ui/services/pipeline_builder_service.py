...
SCENARIO_TRACKING = "Single object tracking"
SCENARIO_ARUCO = "ArUco marker motion"
SCENARIO_CUSTOM = "Custom registry pipeline"
...
TRACKING_ANALYZERS = ("vertical_position",)

TRACKING_SIGNAL_EXTRACTORS = {"opencv_tracker"}
...
def initialise_builder_state(registry) -> None:
    ...
    defaults = {
        ...
        "sef_builder_visualizers": ["matplotlib"],
        "sef_builder_intermediate_visualizers": [],
        "sef_builder_visualizer_targets": {},
        "sef_builder_mode": SCENARIO_TRACKING,
    ...
    st.session_state.setdefault(
        ...
        st.session_state.get("sef_builder_mode", SCENARIO_TRACKING),
    )
...
def builder_state() -> BuilderStateSnapshot:
    ...
    return BuilderStateSnapshot(
        mode=str(st.session_state.get("sef_builder_mode", SCENARIO_TRACKING)),
...
def display_frame_processor_plugin(registry):
    ...
    def _format(name: str) -> str:
        ...
        try:
            plugin = registry.get(category, name)
            return f"{plugin.name} - {plugin_factory_label(plugin)}{suffix}"
        except Exception:
...
def suggested_visualizer_target_indices(
    ...
    compatibility_map = {
        "matplotlib": set(TRACKING_ANALYZERS),
...
def build_pipeline_configuration_from_state() -> PipelineConfiguration:
    ...
    return PipelineConfiguration(
        ...
        analyzers=_build_analyzer_configs(state),
...
def validate_runtime_requirements(config: dict[str, Any]) -> list[str]:
    ...
    if extractor_name == "opencv_tracker":
...
def sync_mode_with_components(mode: str) -> None:
    ...
    if mode == SCENARIO_TRACKING:
        apply_tracking_components()
    elif mode == SCENARIO_ARUCO:
        apply_aruco_components()


def apply_tracking_preset() -> None:
    _disable_json_override()
    st.session_state["sef_builder_mode"] = SCENARIO_TRACKING
    st.session_state[BUILDER_LAST_SYNCED_MODE_KEY] = SCENARIO_TRACKING
    apply_tracking_components()
...
def apply_tracking_components() -> None:
    ...
    st.session_state["sef_builder_analyzers"] = ["vertical_position"]
    st.session_state["sef_builder_frame_processors"] = []
    st.session_state["sef_builder_visualizers"] = ["matplotlib"]


def apply_aruco_preset() -> None:
    _disable_json_override()
    st.session_state["sef_builder_mode"] = SCENARIO_ARUCO
    st.session_state[BUILDER_LAST_SYNCED_MODE_KEY] = SCENARIO_ARUCO
    apply_aruco_components()
...
def _build_analyzer_configs(state: BuilderStateSnapshot) -> tuple[PluginConfig, ...]:
    configs: list[PluginConfig] = []
    for name in state.analyzers:
        if name in TRACKING_ANALYZERS:
