...
class HorizontalFrequencyAnalyzer(IAnalyzer):
    ...
    def analyze(self, signal: ISignal) -> IData:
        ...
        x_positions = np.array(x_values) - np.mean(x_values)
        size = len(x_positions)
        frequency = np.fft.rfftfreq(size, d=dt)
        amplitudes = np.fft.rfft(x_positions)
        ...
        return TwoDimGraphData(
            ...
            label="Horiziontal Frequency Spectrum",
